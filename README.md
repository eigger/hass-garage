...
- **HA → Garage (자동 전송)**: 설치 시 또는 옵션 화면에서 위치(device_tracker)와
  RPM 센서를 고르면, RPM이 바뀔 때마다 그 시점의 최신 위치와 함께 자동으로 Garage에
  전송됩니다. `rest_command` YAML을 직접 쓰지 않아도 됩니다.
- **Garage → HA (자동 수신)**: Garage가 이미 계산해 둔 주행거리, 연료/배터리 잔량,
  마지막 위치, 지난·임박 리마인더를 HA 센서로 보여줍니다.
...
4. Garage 서버 주소(예: `http://192.168.1.50` 또는 `https://garage.example.com`)와,
   ...
   **API 토큰**을 입력합니다.
5. 이어서 전송할 센서(위치 / RPM)를 고르는 화면이 뜹니다 — 지금 안 고르고 비워둬도
   되고, 나중에 옵션에서 언제든 설정할 수 있습니다.
...
주행거리·연료량·속도는 이미 Garage 쪽에서 텔레메트리로부터 계산해 보여주는 값이라
HA에서 다시 보낼 필요가 없어 이 통합구성요소에는 없습니다. 둘 다 선택 사항입니다.

전송 트리거는 **위치를 제외한 나머지 값(RPM)이 바뀔 때만** 걸립니다 — 위치는 주행
중 계속 바뀌므로 트리거에 넣지 않고, 트리거가 걸린 시점의 최신 위치를 함께 실어
보냅니다. 같은 순간에 여러 번 바뀌면 10초 안에 묶어서 한 번만 보내고, 실제 전송은
직렬화되어 있어 여러 전송이 겹쳐도 발생한 순서 그대로 서버에 도착합니다.

정상 동작 여부는 진단 센서 **"마지막 전송 시각"**(HA 로컬 시간 기준)과
**"마지막 전송 상태"**로 확인할 수 있습니다.
